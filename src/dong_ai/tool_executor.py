...
class ToolExecutor:
    ...
    def __init__(self, ceo_mem=None, model_pool=None):
    ...
    def do_web_search(self, query="", q=""):
        q = query or q
    ...
    def do_write_file(self, path="", content=""):
    ...
    def do_browser_screenshot(self, url=""):
        if not url: return "请指定 url"
        from .browser_tool import screenshot
        return screenshot(url)
    
    def do_memory(self, action="", target="memory", content="", key=""):
    ...
    def do_memory_query(self, query="", key=""):
        if not self.ceo_mem: return "记忆系统未初始化"
        q = query or key
